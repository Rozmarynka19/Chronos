...
public class EditBillActivity extends AppCompatActivity implements EditBillBackgroundTaskListener,
    ...
    public void getBillDetails(Hashtable<String, String> tableOfBillDetails) {
        ...
        String[] separateDateTime = Objects.requireNonNull(tableOfBillDetails.get("Bill_Deadline")).split(" ");
        if(separateDateTime.length != 0) {
            paymentDateEdit.setText(separateDateTime[0]);
            paymentTimeEdit.setText(separateDateTime[1]);
        }
    }
}